// SPDX-License-Identifier: MIT
//! Embedded 8×16 bitmap font (ASCII 0x20–0x7E).
//!
//! Each glyph is 16 bytes: one byte per row, MSB = leftmost pixel.
//! Bit set → foreground pixel.  Bit clear → background (or transparent).
//!
//! Font data is derived from the classic public-domain VGA BIOS 8×16 font.
//!
//! # Example
//! ```ignore
//! font::draw_str(
//!     back_buf, stride, screen_h,
//!     10, 10,
//!     "StarOS v0.1",
//!     font::WHITE,
//!     None,       // transparent background
//! );
//! ```

/// Glyph width in pixels.
pub const GLYPH_W: u32 = 8;
/// Glyph height in pixels.
pub const GLYPH_H: u32 = 16;
/// First codepoint covered by the table.
pub const FIRST_CHAR: u8 = 0x20; // space
/// Last codepoint covered by the table.
pub const LAST_CHAR:  u8 = 0x7E; // tilde
/// Number of glyphs in the table.
pub const GLYPH_COUNT: usize = (LAST_CHAR - FIRST_CHAR + 1) as usize; // 95

/// 8×16 glyph bitmap table, indexed by `(ch - FIRST_CHAR)`.
#[rustfmt::skip]
pub static FONT_8X16: [[u8; 16]; GLYPH_COUNT] = [
    // 0x20  space
    [0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00],
    // 0x21  !
    [0x00,0x00,0x18,0x3C,0x3C,0x3C,0x18,0x18,0x18,0x00,0x18,0x18,0x00,0x00,0x00,0x00],
    // 0x22  "
    [0x00,0x66,0x66,0x66,0x24,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00],
    // 0x23  #
    [0x00,0x00,0x6C,0x6C,0xFE,0x6C,0x6C,0x6C,0xFE,0x6C,0x6C,0x00,0x00,0x00,0x00,0x00],
    // 0x24  $
    [0x18,0x18,0x7E,0xC3,0xC2,0xC0,0x7E,0x03,0x83,0xC3,0x7E,0x18,0x18,0x00,0x00,0x00],
    // 0x25  %
    [0x00,0x00,0xC3,0xC6,0x0C,0x18,0x30,0x60,0xC6,0xC3,0x00,0x00,0x00,0x00,0x00,0x00],
    // 0x26  &
    [0x00,0x00,0x38,0x6C,0x6C,0x38,0x76,0xDC,0xCC,0xCC,0x76,0x00,0x00,0x00,0x00,0x00],
    // 0x27  '
    [0x00,0x30,0x30,0x30,0x60,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00],
    // 0x28  (
    [0x00,0x0C,0x18,0x30,0x30,0x30,0x30,0x30,0x30,0x18,0x0C,0x00,0x00,0x00,0x00,0x00],
    // 0x29  )
    [0x00,0x30,0x18,0x0C,0x0C,0x0C,0x0C,0x0C,0x0C,0x18,0x30,0x00,0x00,0x00,0x00,0x00],
    // 0x2A  *
    [0x00,0x00,0x00,0x66,0x3C,0xFF,0x3C,0x66,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00],
    // 0x2B  +
    [0x00,0x00,0x00,0x18,0x18,0x7E,0x18,0x18,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00],
    // 0x2C  ,
    [0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x18,0x18,0x30,0x00,0x00,0x00,0x00],
    // 0x2D  -
    [0x00,0x00,0x00,0x00,0x00,0x00,0x7E,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00],
    // 0x2E  .
    [0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x18,0x18,0x00,0x00,0x00,0x00,0x00],
    // 0x2F  /
    [0x00,0x00,0x03,0x06,0x0C,0x18,0x30,0x60,0xC0,0x00,0x00,0x00,0x00,0x00,0x00,0x00],
    // 0x30  0
    [0x00,0x00,0x3E,0x63,0xC3,0xC3,0xDB,0xC3,0xC3,0x63,0x3E,0x00,0x00,0x00,0x00,0x00],
    // 0x31  1
    [0x00,0x00,0x18,0x38,0x58,0x18,0x18,0x18,0x18,0x18,0x7E,0x00,0x00,0x00,0x00,0x00],
    // 0x32  2
    [0x00,0x00,0x7C,0xC6,0x06,0x0C,0x18,0x30,0x60,0xC6,0xFE,0x00,0x00,0x00,0x00,0x00],
    // 0x33  3
    [0x00,0x00,0x7C,0xC6,0x06,0x06,0x3C,0x06,0x06,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00],
    // 0x34  4
    [0x00,0x00,0x0E,0x1E,0x36,0x66,0xC6,0xFF,0x06,0x06,0x06,0x00,0x00,0x00,0x00,0x00],
    // 0x35  5
    [0x00,0x00,0xFE,0xC0,0xC0,0xC0,0xFC,0x06,0x06,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00],
    // 0x36  6
    [0x00,0x00,0x38,0x60,0xC0,0xC0,0xFC,0xC6,0xC6,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00],
    // 0x37  7
    [0x00,0x00,0xFF,0xC6,0x06,0x0C,0x18,0x30,0x30,0x30,0x30,0x00,0x00,0x00,0x00,0x00],
    // 0x38  8
    [0x00,0x00,0x7C,0xC6,0xC6,0xC6,0x7C,0xC6,0xC6,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00],
    // 0x39  9
    [0x00,0x00,0x7C,0xC6,0xC6,0xC6,0x7E,0x06,0x06,0x0C,0x78,0x00,0x00,0x00,0x00,0x00],
    // 0x3A  :
    [0x00,0x00,0x00,0x18,0x18,0x00,0x00,0x00,0x18,0x18,0x00,0x00,0x00,0x00,0x00,0x00],
    // 0x3B  ;
    [0x00,0x00,0x00,0x18,0x18,0x00,0x00,0x00,0x18,0x18,0x30,0x00,0x00,0x00,0x00,0x00],
    // 0x3C  <
    [0x00,0x00,0x06,0x0C,0x18,0x30,0x60,0x30,0x18,0x0C,0x06,0x00,0x00,0x00,0x00,0x00],
    // 0x3D  =
    [0x00,0x00,0x00,0x00,0x7E,0x00,0x00,0x7E,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00],
    // 0x3E  >
    [0x00,0x00,0x60,0x30,0x18,0x0C,0x06,0x0C,0x18,0x30,0x60,0x00,0x00,0x00,0x00,0x00],
    // 0x3F  ?
    [0x00,0x00,0x7C,0xC6,0xC6,0x0C,0x18,0x18,0x00,0x18,0x18,0x00,0x00,0x00,0x00,0x00],
    // 0x40  @
    [0x00,0x00,0x7C,0xC6,0xC6,0xDE,0xDE,0xDE,0xDC,0xC0,0x7C,0x00,0x00,0x00,0x00,0x00],
    // 0x41  A
    [0x00,0x00,0x10,0x38,0x6C,0xC6,0xC6,0xFE,0xC6,0xC6,0xC6,0x00,0x00,0x00,0x00,0x00],
    // 0x42  B
    [0x00,0x00,0xFC,0x66,0x66,0x66,0x7C,0x66,0x66,0x66,0xFC,0x00,0x00,0x00,0x00,0x00],
    // 0x43  C
    [0x00,0x00,0x3C,0x66,0xC2,0xC0,0xC0,0xC0,0xC2,0x66,0x3C,0x00,0x00,0x00,0x00,0x00],
    // 0x44  D
    [0x00,0x00,0xF8,0x6C,0x66,0x66,0x66,0x66,0x66,0x6C,0xF8,0x00,0x00,0x00,0x00,0x00],
    // 0x45  E
    [0x00,0x00,0xFE,0x66,0x62,0x68,0x78,0x68,0x62,0x66,0xFE,0x00,0x00,0x00,0x00,0x00],
    // 0x46  F
    [0x00,0x00,0xFE,0x66,0x62,0x68,0x78,0x68,0x60,0x60,0xF0,0x00,0x00,0x00,0x00,0x00],
    // 0x47  G
    [0x00,0x00,0x3C,0x66,0xC2,0xC0,0xDE,0xC6,0xC6,0x66,0x3A,0x00,0x00,0x00,0x00,0x00],
    // 0x48  H
    [0x00,0x00,0xC6,0xC6,0xC6,0xC6,0xFE,0xC6,0xC6,0xC6,0xC6,0x00,0x00,0x00,0x00,0x00],
    // 0x49  I
    [0x00,0x00,0x3C,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x3C,0x00,0x00,0x00,0x00,0x00],
    // 0x4A  J
    [0x00,0x00,0x1E,0x0C,0x0C,0x0C,0x0C,0x0C,0xCC,0xCC,0x78,0x00,0x00,0x00,0x00,0x00],
    // 0x4B  K
    [0x00,0x00,0xE6,0x66,0x6C,0x78,0x70,0x78,0x6C,0x66,0xE6,0x00,0x00,0x00,0x00,0x00],
    // 0x4C  L
    [0x00,0x00,0xF0,0x60,0x60,0x60,0x60,0x60,0x62,0x66,0xFE,0x00,0x00,0x00,0x00,0x00],
    // 0x4D  M
    [0x00,0x00,0xC3,0xE7,0xFF,0xFF,0xDB,0xC3,0xC3,0xC3,0xC3,0x00,0x00,0x00,0x00,0x00],
    // 0x4E  N
    [0x00,0x00,0xC6,0xE6,0xF6,0xFE,0xDE,0xCE,0xC6,0xC6,0xC6,0x00,0x00,0x00,0x00,0x00],
    // 0x4F  O
    [0x00,0x00,0x7C,0xC6,0xC6,0xC6,0xC6,0xC6,0xC6,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00],
    // 0x50  P
    [0x00,0x00,0xFC,0x66,0x66,0x66,0x7C,0x60,0x60,0x60,0xF0,0x00,0x00,0x00,0x00,0x00],
    // 0x51  Q
    [0x00,0x00,0x7C,0xC6,0xC6,0xC6,0xC6,0xC6,0xC6,0xCE,0x7C,0x0E,0x00,0x00,0x00,0x00],
    // 0x52  R
    [0x00,0x00,0xFC,0x66,0x66,0x66,0x7C,0x78,0x6C,0x66,0xE6,0x00,0x00,0x00,0x00,0x00],
    // 0x53  S
    [0x00,0x00,0x7C,0xC6,0xC6,0x60,0x38,0x0C,0xC6,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00],
    // 0x54  T
    [0x00,0x00,0xFF,0xDB,0x99,0x18,0x18,0x18,0x18,0x18,0x3C,0x00,0x00,0x00,0x00,0x00],
    // 0x55  U
    [0x00,0x00,0xC6,0xC6,0xC6,0xC6,0xC6,0xC6,0xC6,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00],
    // 0x56  V
    [0x00,0x00,0xC3,0xC3,0xC3,0xC3,0xC3,0xC3,0x66,0x3C,0x18,0x00,0x00,0x00,0x00,0x00],
    // 0x57  W
    [0x00,0x00,0xC3,0xC3,0xC3,0xC3,0xDB,0xDB,0xFF,0x66,0x66,0x00,0x00,0x00,0x00,0x00],
    // 0x58  X
    [0x00,0x00,0xC3,0xC3,0x66,0x3C,0x18,0x3C,0x66,0xC3,0xC3,0x00,0x00,0x00,0x00,0x00],
    // 0x59  Y
    [0x00,0x00,0xC3,0xC3,0xC3,0x66,0x3C,0x18,0x18,0x18,0x3C,0x00,0x00,0x00,0x00,0x00],
    // 0x5A  Z
    [0x00,0x00,0xFF,0xC3,0x86,0x0C,0x18,0x30,0x60,0xC1,0xFF,0x00,0x00,0x00,0x00,0x00],
    // 0x5B  [
    [0x00,0x00,0x3C,0x30,0x30,0x30,0x30,0x30,0x30,0x30,0x3C,0x00,0x00,0x00,0x00,0x00],
    // 0x5C  backslash
    [0x00,0x00,0xC0,0x60,0x30,0x18,0x0C,0x06,0x03,0x00,0x00,0x00,0x00,0x00,0x00,0x00],
    // 0x5D  ]
    [0x00,0x00,0x3C,0x0C,0x0C,0x0C,0x0C,0x0C,0x0C,0x0C,0x3C,0x00,0x00,0x00,0x00,0x00],
    // 0x5E  ^
    [0x10,0x38,0x6C,0xC6,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00],
    // 0x5F  _
    [0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0x00,0x00,0x00,0x00,0x00],
    // 0x60  `
    [0x30,0x18,0x0C,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00],
    // 0x61  a
    [0x00,0x00,0x00,0x00,0x78,0x0C,0x7C,0xCC,0xCC,0xCC,0x76,0x00,0x00,0x00,0x00,0x00],
    // 0x62  b
    [0x00,0x00,0xE0,0x60,0x60,0x78,0x6C,0x66,0x66,0x66,0x7C,0x00,0x00,0x00,0x00,0x00],
    // 0x63  c
    [0x00,0x00,0x00,0x00,0x7C,0xC6,0xC0,0xC0,0xC0,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00],
    // 0x64  d
    [0x00,0x00,0x1C,0x0C,0x0C,0x3C,0x6C,0xCC,0xCC,0xCC,0x76,0x00,0x00,0x00,0x00,0x00],
    // 0x65  e
    [0x00,0x00,0x00,0x00,0x7C,0xC6,0xFE,0xC0,0xC0,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00],
    // 0x66  f
    [0x00,0x00,0x1C,0x36,0x32,0x30,0x7C,0x30,0x30,0x30,0x78,0x00,0x00,0x00,0x00,0x00],
    // 0x67  g
    [0x00,0x00,0x00,0x00,0x76,0xCC,0xCC,0xCC,0x7C,0x0C,0xCC,0x78,0x00,0x00,0x00,0x00],
    // 0x68  h
    [0x00,0x00,0xE0,0x60,0x6C,0x76,0x66,0x66,0x66,0x66,0xE6,0x00,0x00,0x00,0x00,0x00],
    // 0x69  i
    [0x00,0x00,0x18,0x18,0x00,0x38,0x18,0x18,0x18,0x18,0x3C,0x00,0x00,0x00,0x00,0x00],
    // 0x6A  j
    [0x00,0x06,0x06,0x00,0x06,0x06,0x06,0x06,0x06,0x66,0x66,0x3C,0x00,0x00,0x00,0x00],
    // 0x6B  k
    [0x00,0x00,0xE0,0x60,0x66,0x6C,0x78,0x78,0x6C,0x66,0xE6,0x00,0x00,0x00,0x00,0x00],
    // 0x6C  l
    [0x00,0x00,0x38,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x3C,0x00,0x00,0x00,0x00,0x00],
    // 0x6D  m
    [0x00,0x00,0x00,0x00,0xE6,0xFF,0xDB,0xDB,0xDB,0xDB,0xDB,0x00,0x00,0x00,0x00,0x00],
    // 0x6E  n
    [0x00,0x00,0x00,0x00,0xDC,0x66,0x66,0x66,0x66,0x66,0x66,0x00,0x00,0x00,0x00,0x00],
    // 0x6F  o
    [0x00,0x00,0x00,0x00,0x7C,0xC6,0xC6,0xC6,0xC6,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00],
    // 0x70  p
    [0x00,0x00,0x00,0x00,0xDC,0x66,0x66,0x66,0x7C,0x60,0x60,0xF0,0x00,0x00,0x00,0x00],
    // 0x71  q
    [0x00,0x00,0x00,0x00,0x76,0xCC,0xCC,0xCC,0x7C,0x0C,0x0C,0x1E,0x00,0x00,0x00,0x00],
    // 0x72  r
    [0x00,0x00,0x00,0x00,0xDC,0x76,0x66,0x60,0x60,0x60,0xF0,0x00,0x00,0x00,0x00,0x00],
    // 0x73  s
    [0x00,0x00,0x00,0x00,0x7C,0xC6,0x60,0x38,0x0C,0xC6,0x7C,0x00,0x00,0x00,0x00,0x00],
    // 0x74  t
    [0x00,0x00,0x10,0x30,0x30,0xFC,0x30,0x30,0x30,0x36,0x1C,0x00,0x00,0x00,0x00,0x00],
    // 0x75  u
    [0x00,0x00,0x00,0x00,0xCC,0xCC,0xCC,0xCC,0xCC,0xCC,0x76,0x00,0x00,0x00,0x00,0x00],
    // 0x76  v
    [0x00,0x00,0x00,0x00,0xC3,0xC3,0xC3,0xC3,0x66,0x3C,0x18,0x00,0x00,0x00,0x00,0x00],
    // 0x77  w
    [0x00,0x00,0x00,0x00,0xC3,0xC3,0xC3,0xDB,0xDB,0xFF,0x66,0x00,0x00,0x00,0x00,0x00],
    // 0x78  x
    [0x00,0x00,0x00,0x00,0xC3,0x66,0x3C,0x18,0x3C,0x66,0xC3,0x00,0x00,0x00,0x00,0x00],
    // 0x79  y
    [0x00,0x00,0x00,0x00,0xC6,0xC6,0xC6,0xC6,0x7E,0x06,0x0C,0xF8,0x00,0x00,0x00,0x00],
    // 0x7A  z
    [0x00,0x00,0x00,0x00,0xFF,0x66,0x0C,0x18,0x30,0x66,0xFF,0x00,0x00,0x00,0x00,0x00],
    // 0x7B  {
    [0x00,0x00,0x0E,0x18,0x18,0x18,0x70,0x18,0x18,0x18,0x0E,0x00,0x00,0x00,0x00,0x00],
    // 0x7C  |
    [0x00,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x00,0x00,0x00,0x00],
    // 0x7D  }
    [0x00,0x00,0x70,0x18,0x18,0x18,0x0E,0x18,0x18,0x18,0x70,0x00,0x00,0x00,0x00,0x00],
    // 0x7E  ~
    [0x76,0xDC,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00],
];

// ---------------------------------------------------------------------------
// Rendering
// ---------------------------------------------------------------------------

/// Draw one character glyph into a pixel buffer.
///
/// * `buf`    — back-buffer slice (stride × height elements).
/// * `stride` — pixels per row in `buf`.
/// * `height` — buffer height in rows (for bounds checking).
/// * `x`, `y` — top-left corner of the glyph.
/// * `ch`     — ASCII codepoint to render (non-printable → rendered as space).
/// * `fg`     — foreground color (ARGB8888).
/// * `bg`     — background color; `None` = transparent (skip background pixels).
pub fn draw_char(
    buf:    &mut [u32],
    stride: u32,
    height: u32,
    x:      u32,
    y:      u32,
    ch:     u8,
    fg:     u32,
    bg:     Option<u32>,
) {
    let glyph = glyph_for(ch);

    for row in 0..GLYPH_H {
        let py = y + row;
        if py >= height { break; }

        let bits = glyph[row as usize];
        let row_base = (py * stride) as usize;

        for col in 0..GLYPH_W {
            let px = x + col;
            if px >= stride { break; } // also clips to width (stride == width normally)

            let bit_set = bits & (0x80 >> col) != 0;
            let pixel = if bit_set {
                fg
            } else if let Some(bg_color) = bg {
                bg_color
            } else {
                continue; // transparent
            };

            let idx = row_base + px as usize;
            if idx < buf.len() {
                buf[idx] = pixel;
            }
        }
    }
}

/// Draw a string of ASCII text into a pixel buffer.
///
/// Newlines (`\n`) wrap to the next line.
/// Returns the `(x, y)` position after the last character.
pub fn draw_str(
    buf:    &mut [u32],
    stride: u32,
    height: u32,
    x:      u32,
    y:      u32,
    s:      &str,
    fg:     u32,
    bg:     Option<u32>,
) -> (u32, u32) {
    let mut cx = x;
    let mut cy = y;

    for byte in s.bytes() {
        if byte == b'\n' {
            cx  = x;
            cy += GLYPH_H;
            continue;
        }
        if cy + GLYPH_H > height { break; }

        draw_char(buf, stride, height, cx, cy, byte, fg, bg);
        cx += GLYPH_W;

        // Wrap at right edge.
        if cx + GLYPH_W > stride {
            cx  = x;
            cy += GLYPH_H;
        }
    }

    (cx, cy)
}

/// Returns the width in pixels of `s` rendered with this font (no wrapping).
#[inline]
pub fn text_width(s: &str) -> u32 {
    s.bytes().filter(|&b| b != b'\n').count() as u32 * GLYPH_W
}

// ---------------------------------------------------------------------------
// Internal helpers
// ---------------------------------------------------------------------------

#[inline]
fn glyph_for(ch: u8) -> &'static [u8; 16] {
    if ch >= FIRST_CHAR && ch <= LAST_CHAR {
        &FONT_8X16[(ch - FIRST_CHAR) as usize]
    } else {
        &FONT_8X16[0] // fallback: space
    }
}
